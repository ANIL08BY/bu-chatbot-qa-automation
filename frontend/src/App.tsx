...
const GREETING = 'Merhaba! Ben BU AI, Belek Üniversitesi\'nin sanal asistanıyım. Bugün size nasıl yardımcı olabilirim?';
...
export default function App() {
  ...
  const [isFullscreen, setIsFullscreen] = useState(
    () => !!document.fullscreenElement || window.innerHeight === window.screen.height
  );
  ...
  const handleNewChat = () => {
    ...
    const fresh = [{ id: Date.now().toString(), text: GREETING, isUser: false, isGreeting: true }];
  ...
  return (
    <div className={`flex flex-col h-screen w-full relative ${isDarkMode ? 'bg-[#1a1a1a]' : 'bg-[#f4f6f9]'}`} role="main">
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto px-3 sm:px-6 py-6 sm:py-8 relative z-10">
            ...
            <ChatMessage
              ...
              onFeedback={
                !message.isUser && !message.isError && !message.isGreeting
                  ? (value) => handleFeedback(message.id, message.messageDbId, value, message.feedback ?? null)
                  : undefined
              }
            />
          ))}
          ...
          {isTyping && (
            <div className="flex justify-start mb-4 sm:mb-5">
              <div className="flex gap-2.5">
                ...
                <div className={`rounded-2xl px-5 py-3 ${isDarkMode ? 'bg-[#3a3a3a]' : 'bg-white shadow-sm'}`}>
