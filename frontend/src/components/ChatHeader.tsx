...
export function ChatHeader({ title = 'Belek AI', onNewChat, onSettingsClick, isDarkMode = false }: ChatHeaderProps) {
  const hoverBg = isDarkMode ? 'hover:bg-[#7a0e0d]' : 'hover:bg-[#9b1211]';

  return (
    <header className={`relative px-3 sm:px-6 py-2 flex items-center justify-between gap-2 sm:gap-4 shadow-md ${
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
        ...
        />
        <h1 className="text-xl sm:text-2xl font-bold text-white leading-tight truncate ml-1 sm:ml-2" style={{ fontFamily: "'Oxanium', sans-serif" }}>{title}</h1>
      </div>
